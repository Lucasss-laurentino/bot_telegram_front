...
export const CreateMorador = () => {
  ...
  const { createMorador, loading, erro, setErro, setInitialData } =
    useMorador();
  const [imagem, setImagem] = useState<File | null>(null);
  const [popupActive, setPopupActive] = useState(false);

  const handleMorador = async (novoMorador: INovoMorador) => {
    var confirmacao = await createMorador(novoMorador);
    if (confirmacao) {
      reset();
      setImagem(null);
      setPopupActive(true);
  ...
  useEffect(() => {
    ...
    //setInitialData("tgWebAppData");
  }, []);

  return (
    <>
      <Popup active={popupActive} message="Morador criado com sucesso!" />
      <div className="morador-page">
        <div className="morador-card">
          ...
          <form className="morador-form" onSubmit={handleSubmit(handleMorador)}>
            ...
            {errors.Celular?.message && (
              <Errors error={errors.Celular.message} />
            )}
